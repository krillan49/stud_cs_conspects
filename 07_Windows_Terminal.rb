...
# Терминал - это это текстовый интерфейс операционной системы
...
# 1. Windows PowerShell (современные версии Виндоус) - добавляет имена многих команд cmd.exe и Bash к своим командам для удобства, так же использует полные имена(1й столбец). Находится: значек пуска -> правая кнопка -> выбираем обычный или администратор
# 2. cmd.exe/COMMAND.COM (MS-DOS, Windows 10 и ниже). Находится: выполнить -> cmd
# 3. Bash (Unix, BSD, Linux, Mac OS, Git Bash, ...)

# Если в поиске Windows начать вводить cmd то выведет все варианты установленных командных строк
...
# Когда вводим команду и имя какого-то фаила или директории достаточно ввести его уникальную часть и нажать "tab"
...
# Ctrl+d    - (обозначается также ^D) конец ввода, end of input, end of file, EOF
...
# Список и описание универсальных(по возможности) команд для PowerShell, cmd и Bash:
...
# 4.  clear - команда, чтобы очистить экран терминала
# PowerShell              - cls, clear
# cmd.exe/COMMAND.COM     - cls
# Bash                    - clear


# 5.  mkdir - команда для cоздания новой директории
# PowerShell              -
# cmd.exe/COMMAND.COM     -
# Bash                    -

# > mkdir имя_директории                 - cоздать новую директорию в текущей директории
# > mkdir имя_1, имя_2, имя_3            - можно создавать одновременно множество директорий указывая их через запятую


# 6.  ren - команда, чтобы переименовать директорию (в Bash нету, применяется mv)
# PowerShell              - mi, ren
# cmd.exe/COMMAND.COM     - ren, rename
# Bash                    - mv

# > ren старое_имя новое_имя                   - переименовать директорию


# 7.  rm - команда удаления фаила или директории(через терминал все удаляется безвозвратно, нет корзины)
# PowerShell              - ri, del, erase, rmdir, rd, rm
# cmd.exe/COMMAND.COM     - del, erase, rmdir, rd
# Bash                    - rmdir, rm

# > rm имя_объекта                       - удаление фаила или директории
# > rm имя_1, имя_2                      - удаление одновременно нескольких объектов
# > rm -rf /                             - !!! удалит содержимое корневой директории на диске без какого-либо подтверждения.


# 8.  man - команда справки по указанной в параметре команде (выйти из меню справки Ctrl+C)
# PowerShell              - help, man
# cmd.exe/COMMAND.COM     - help
# Bash                    - man

# > man имя_команды                  - показать справку


# 9.  ni - команда создания нового фаила
# PowerShell              -
# cmd.exe/COMMAND.COM     -
# Bash                    -

# > ni имя.расширение                           - создание нового фаила
# > ni папка\имя.расширение


# 10. cp - команда копирования фаила или папки
...
# > cp имя_фаила.расширение имя_папки                             - копируем фаил или папку "имя_фаила" в папку "имя_папки"
...
# 11. mv - команда чтоьы переместить(вырезать+копировать) фаил или папку (не нужен рекурсивный перенос, переносится с содержимым и так)
# PowerShell              - mi, move, mv
# cmd.exe/COMMAND.COM     - move
# Bash                    - mv

# > mv имя_фаила.расширение имя_папки                               - переместить фаил или папку "имя_фаила" в папку "имя_папки"
...
# 14. echo - команда выведет текст из параметра (параметр в кавычках строка, без кавычек массив)
# PowerShell               - echo, write
# cmd.exe/COMMAND.COM      - echo ??
# Bash                     - echo

# > echo 'какойто текст'   - выведет текст 'какойто текст'
# > echo %PATH%            - показывает все пути переменных среды (работает только в старом терминале винды)


# 15. cat - команда, чтобы открыть какой-то фаил и вывести содержимое в терминал
# PowerShell              - gc, type, cat
# cmd.exe/COMMAND.COM     - type
# Bash                    - cat

# > cat путь\имя_фаила.расширение               - открыть фаил и вывести содержимое в терминал
...
# 2. Открываем (например блокнотом или IDE) и пишем в нем любые команды (каждую с новой строки)
