...
# HTTP - набор байтов (текст) построенный по определенному принципу, в соответсвии с правилами этого протокола

# HTTP зпрос состоит из 3х частей:
...
# HTTP ответ сервера состоит из этих же 3х частей
# 1. Стартовая линия (строка статуса) - содержит код ответа, число позволяет клиенту понять что произошло
# 2. Заголовки - содржат служебную информацию: тип ОС, браузер итд
# 3. Тело запроса - содержит информацию
