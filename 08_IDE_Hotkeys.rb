...
# tab+shift           - отступ влево (тоесть удаление отступа)
...
# shift+F4  - создать фаил. Появится Editor(редактор). После нажатия enter созданный фаил откроется
...
# Настройка отступов(тут для Руби меняем таб на 2 пробела):
...
# Переназначить комбинацию клавиш в VS Code:
# 1. `File` -> `Preferences` -> `Keyboard Shortcuts` (или нажмите `Ctrl+K Ctrl+S`).
# 2. В строке поиска введите "Comment Line".
# 3. Найдите команду "Editor: Toggle Line Comment".
# 4. Дважды щелкните по текущей комбинации клавиш (или щелкните правой кнопкой мыши и выберите "Change Keybinding").
# 5. В появившемся диалоговом окне нажмите желаемую комбинацию клавиш, которая будет работать независимо от раскладки (например, `Ctrl+/` или `Ctrl+K Ctrl+C`). Убедитесь, что эта комбинация не занята другой командой.  VS Code сообщит вам, если есть конфликт.
# 6. Нажмите `Enter`, чтобы сохранить новую комбинацию.
...
# Открыть/скрыть скрытые/игнорируемые фаилы и папки:
# Settings → Packeges, в поиске Tree View -> ставим или снимаем галочки необходимые
