...
// В большинстве языков программирования нет множественного наследования, тоесть не можем наследовать сразу от нескольких классов. В тех же языках где множественное наследование есть, например C++, класс наследник примет все свойства и методы от всех классов у которых он наследует.
...
class Database {
  ...
  constructor() {
    if (Database.instance) { // тоесть если эта переменная не равно null, а содержит объект БД, тоесть объект БД уже существует, то возвращаем его
