...
// Функциональное программирование(ФП) - это одна из основных парадигм программирования.

// ФП - менее популярная парадигма, с точки зрения бизнеса, чем ООП

// Не все языки могут применить ФП: есть чисто ФП-языки(Haskell, Lisp, F#), есть мультипарадигменные языки(JS, Scala) и есть чисто ООП-языки

// ФП основано на том, что мы управляем функциями как математическими функциями(лямбда вычисления). Тоесть к функциям в ФП нужно относиться не как к набору действий(процедуры), а как к результату вычислений, из чего и следуют все дальнейшие свойства и принципы ФП

// Четыре основных концепции вытекающие из ФП:
// 1. Декларативный подход в написании кода
// 2. Чистые функции - при одних и тех же переданных параметрах будет возвращаться всегда один и тот же результат
...
// Преимущества ФП - небольшие функции, которые делают много работы. Они комбинируются в более сложные операции. Все органично строится из простых функций и их композиций и комбинаторов. map/filter/fold/scan и все прочее. Очень мало состояния. Мало мутабельности. По месту вызова функции - видно весь контекст, тк все от чего функция зависит это ее аргументы. То же самое по месту возврата. Очень простой последовательный код, где идут простые последовательные трансформации над данными. Очень просто читать, понимать, выполнять в голове



//                                            Процедура и функция (?? потом вынести кудато ??)
...
// Процедура обычно представляет собой подпрограмму, которая выполняет некоторые действия, не возвращая значения. В отличие от функции, процедура не предназначена для того, чтобы возвращать результат, хотя она может изменять состояние или производить вывод.
// В некоторых языках, если просто написать блок кода (функцию), которая не использует `return` для возвращения значения, это будет рассматриваться скорее как процедура. По сути, любой определённый вами блок в функции можно считать процедурой, если он просто выполняет действия

// Функции - это обособленный фрагмент кода(подпрогрпмма). В нее могут быть переданны данные для работы(параметры). Функция может опционально возвращать данные (возвращаемое значение). Все данные, передаваемые функции, передаются явно.
...
// 1. Императивный стиль - описываем действие(описываем как мы хотим что-то получить, те каждое действие ведущее к результату)
...
function calcUsersSumAgeByCountry(users, country) {
  ...
  for (let i = 0; i < filteredUsers.length; i++) {
    sum += filteredUsers[i].age;                                                     // считаем общий возраст
  }
  return sum;
}
...
function filterByCountry(users, country) { // Отдельная функция фильтрует по стране (это "подкапотный" имеративный код)
  const filteredUsers = [];
  for (let i = 0; i < users.length; i++) {
    if (users[i].country === country) filteredUsers.push(users[i]);
  }
...
// 4. Еще более декларативный вариант с выносом функций-аргументов как функций общего назначения, которые могут преобразовывать данные по разным полям.
...
const MODIFER = 5; // результат будет зависеть от внешней переменной, которую ктото может изменить
function mult(a, b) { return a * b * MODIFER; }

async function mult(a, b) { // результат будет зависеть от стороннего действия
  const modifer = await fetch(input: 'get/modifer'); // сайд эффект, запрос с бэкэнда
  return a * b * modifer;
}

let modifer = 5; // результат будет зависеть от внешней переменной, которую наша функция может изменить
function mult(a, b) {
...
// передать в другую функцию - коллбек (функция обратного вызова), тоесть функция, которая передается чтобы быть вызванной потом;
...
// Функция высшего порядка - это функция, которая принимает аргументами другие функции (коллбеки) или возвращает другую функцию. Принимая по цепочке функций аргументами другие функции, очень сложно сделать код не чистым или данные мутируемыми, тк сразу все сломается и не будет работать.
// Функция высшего порядка это мощный инструмент, дающий большую гибкость

// Пример1: в ЖС функции map(), filter() итд это функции высшего порядка, тк они принимают и используют другие функции в качестве аргумента (колбэки). Пример как работает map():
function myMap(callback) {
  const res = [];
  for (let i = 0; i < this.length; i++) {
    res.push(callback(this[i])); // тоесть использует функцию-аргумент для каждого элемента
  }
  return res;
}
...
// Композиция и конвейер функций - это вспомогательные инструменты, которые позволяют комбинировать фунции в составную функцию, чтобы они выполнялись в определенной последовательности.
...
const operator = { roles: ['ADMIN', 'MANAGER'] };
...
// Но для пользователей, что очень часто встречаются в системе, хочется передавать не 2 аргумента, а 1, для чего создадим дополнительные функции-обертки, в которых уже будут переданы конкретные типы пользователей:
...
// Можно создать функции обертки на предыдущими функциями обертками, в которые вообще не нужно передавать аргументов, для самых часто встречаемых комбинаций пользователей и ролей
...
//                                       Каррирование. Множественные аргументы
...
function curry(func) {
  ...
  return function curried(...args) { // сюда принимаем аргументы из первого вызова curriedSum(5)(4)(3)
    if (func.length <= args.length) { // если количество аргуметов принимаемых функцией sum меньше или равно количеству аргументов, переданных в эту каррированную функцию
...
// Нужен чтобы создавать копии объектов, вместо передачи ссылок на старый и данные были иммутабельными ??

// Контейнер - это например класс, который инкапсулирует в себе какое-то значение. Те если рассматривать пример ниже - содержит статический метод of

// Тут синтаксически код для создания контейнера будет классом. При этом класс стоит воспринимить как синтаксический сахар, а не как парадигму ООП, тоже самое можно было сделать и при помощи прототипов и замыканий, просто так проще код. Парадигма не зависит от того при помощи какого синтаксиса мы пишем код, а только от того какким мы его пишем
...
class Container { // Класс-контейнер объектами которого будут объекты-контейнеры
  ...
  map(fn) {
    return Container.of(fn(this.$value)); // fn(this.$value) результат этой функции будет свойством нового контейнера, тоесть он будет запакован в новый контейнер и этот новый контейнер будет возвращен и от него можно будет еще раз вызвать метод map
  }
...
Container.of(5).$value; //=> 5                          // Чтобы такого не было конструктор должен быть приватным
...
// Функтор похож на интерфейс из ООП, который описывает, что наследуемые от него классы должны обладать методом map
...
// Монада - это что-то вроде имплементации функтора-"интерфейса", тоесть конкретная реализация функтора с какой-то определенной логикой для определенных задач
...
class Maybe {
  ...
  ap(functor) { // принимает аргументом другой функтор
    ...
    // this.$value - значение функтора от которого вызвали ap, а передаем его в map вызванное уже функтором аргументом
  }
  map(fn) {
