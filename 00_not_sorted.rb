...
puts '                                            Поток и процесс'

# процесс — это контейнер для потоков, а потоки — это отдельные пути выполнения внутри этого контейнера

# 1. Процесс - это экземпляр программы, который выполняется в операционной системе. Каждый процесс имеет свое собственное пространство памяти, включая код программы, данные и стек.
# Процессы изолированы друг от друга, что обеспечивает безопасность и стабильность системы.
# Процессы могут взаимодействовать друг с другом через механизмы межпроцессного взаимодействия (IPC), такие как очереди сообщений, семафоры и т.д.

# 2. Поток (или нить) - это более легковесная единица выполнения, которая существует внутри процесса. Потоки делят между собой память и ресурсы процесса, что позволяет им взаимодействовать более эффективно.
# Каждый поток имеет свой собственный стек и регистры, но использует общее пространство памяти процесса.
# Поскольку потоки легче создаются и управляются по сравнению с процессами, они часто используются для выполнения параллельных задач внутри одного приложения.
